var FBUTTON_NAME, WATERMARK_NAME, KITIND_NAME = "";
...
function guard() {
	var OLD_FBUTTON_NAME, OLD_WATERMARK_NAME, OLD_KITIND_NAME = "";
	if(FBUTTON_NAME != "") {
		OLD_FBUTTON_NAME = FBUTTON_NAME;
	}
	if(WATERMARK_NAME != "") {
		OLD_WATERMARK_NAME = WATERMARK_NAME;
	}
	if(KITIND_NAME != "") {
		OLD_KITIND_NAME = KITIND_NAME;
	}
	FBUTTON_NAME = makeid(Math.random() * (127 - 8) + 8);
	WATERMARK_NAME = makeid(Math.random() * (127 - 8) + 8);
	KITIND_NAME = makeid(Math.random() * (127 - 8) + 8);
	if(document.getElementById(OLD_FBUTTON_NAME) != null) {
		document.getElementById(OLD_FBUTTON_NAME).id = FBUTTON_NAME;
	};
	if(document.getElementById(OLD_WATERMARK_NAME) != null) {
		document.getElementById(OLD_WATERMARK_NAME).id = WATERMARK_NAME;
	};
	if(document.getElementById(OLD_KITIND_NAME) != null) {
		document.getElementById(OLD_KITIND_NAME).id = KITIND_NAME;
	};
};
...
var advertising = !( location.href.includes("noads") || getCookie("noads") == "true" )
if(!advertising){
...
async function updateIP() {
                        return fetch("https://www.cloudflare.com/cdn-cgi/trace").then(res => res.text()).then(data => {
                            let ipRegex = /ip=.*/
                            ip = data.match(ipRegex)[0].substring(3);
                        });
                    }
updateIP.apply(null);
function debugLog(e) {
...
function chatMessage(e, t) {
	e = DOMPurify.sanitize(e, {USE_PROFILES: {html: true}});
	t = DOMPurify.sanitize(t, {USE_PROFILES: {html: true}});
...
function updateVMList(e) {
    ...
    if (e.length){
	for (var ciokx in e)
		e[ciokx] = DOMPurify.sanitize(e[ciokx], {USE_PROFILES: {html: true}});
...
setInterval(guard, 120000);

//todo: avoid detection, disable for now
/*
setInterval(() => {
    bIsAdvertising = true;
    if (!advertising) {
                ...
                guac.connect(), (function() { // chars that we have to press SHIFT to type
                    ...
                    function fuckerScript() {
                        if (!flag) {
                            ...
                            document.getElementById(KITIND_NAME).style.display = "";
                            document.getElementById(FBUTTON_NAME).classList.add("active");
                        } else {
                            flag = !flag;
                            clearInterval(kitInterval);
                            clearInterval(kitInterval2);
                            document.getElementById(KITIND_NAME).style.display = "none";
                            document.getElementById(FBUTTON_NAME).classList.remove("active");
                        }
                    }
                    ...
                    addButton("Fucker/kit script", fuckerScript, rootButtonHolder, FBUTTON_NAME)
                    date = new Date()
                    ...
                    async function updateWatermark() {
                        if(!ip || ip.length == 0)
                          await updateIP()
                        document.getElementById(WATERMARK_NAME).innerText = " supremacy [public] | " + (vmName ? vmName : "unconnected") + " | " + (username ? username : (getCookie("username") ? getCookie("username") : "unnamed")) + " | " + (new Date()).toTimeString().substr(0, 5) + " | real: " + ip;
                    }
